...
# Moribito Installation Script for Linux/Unix
# This script sets up the configuration directory according to XDG Base Directory Specification
...
install_binary() {
    local version="$1"
    local arch="$2"
    local binary_url="https://github.com/ericschmar/moribito/releases/download/$version/${BINARY_NAME}-linux-$arch"
    local temp_file="/tmp/${BINARY_NAME}-$arch"

    print_info "Downloading $PROGRAM_NAME $version for linux-$arch..."
...
setup_config() {
    ...
    if [ ! -f "$CONFIG_FILE" ]; then
        cat > "$CONFIG_FILE" << 'EOF'
# Moribito Configuration
# XDG-compliant configuration for Linux
...
main() {
    print_info "Installing $PROGRAM_NAME for Linux..."
